...
def set_update_log(conn, code, type,end):
    cursor = conn.cursor()
...
def save_share(conn, code, autype):
    ...
    if not is_last_update_day(conn, code, "hfq"):
        ...
        share_data = None
        if end_date is None:
            share_data = share_tool.get_h_data(code, start=time_to_market_day, end=now_day, autype="hfq",in_batch=True,pause=pause)
            print(code + "已从网络下载(hfq),从"+time_to_market_day+"到"+now_day)
        else:
            share_data = share_tool.get_h_data(code, start=end_date, end=now_day, autype="hfq",in_batch=True,pause=pause)
            print(code + "已从网络下载(hfq),从" + end_date + "到" + now_day)
        #保存dataframe数据至库表
        util.save_dataframe(conn=conn, data=share_data, table_name="hfq_share", primary_key="date,code,autype",save_index=True)
        end = util.get_object(conn,"select date from hfq_share where code = '"+code+"'")
        # 更新log日期
        set_update_log(conn, code, "hfq",end)
        print(code + "已更新至" + now_day)
